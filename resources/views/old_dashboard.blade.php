...
$get_file_no=DB::table('properties')->where('user_id', auth()->user()->id)->latest()->first();
$file_no=1;
if($get_file_no)
{
    ...
    <div class="container h-100">
        <div class="row pt-10 pb-10 ms-10 h-100 align-items-center">
            ...
            <div class="col text-center scrapper-completed-button d-none">
                ...
                <a href="{{route('my-properties')}}" class="btn btn-two">Properties</a>
            </div>
        </div>
    </div>
</div>
                                ...
                                <div class="stripe-form-div d-none">
                                    ...
                                    <form class="pt-20" action="{{ route('manuall-payment') }}" method="post" id="payment-form">
                                        ...
                                        <div class="form-row pb-60">
                                            ...
                                            <input type="hidden" id="card_holder_name" value="{{auth()->user()->name}}"
                                            ...
                                            <input type="hidden" id="card_holder_email" value="{{auth()->user()->email}}"
...
<script>
    const userTotalCredits = "{{$userTotalCredits}}";
    ...
    function showStripe() {
        ...
        if (amount && amount > 0 && obtained_credits >= totalRowsData) {
    ...
    const availableCredits = '{{auth()->user()->credit}}';
    var totalRowsData = 0;
    ...
    function showConfirmation() {
        ...
        if (csvData) {
            csvData.headers.forEach(header => {
                ...
                // Create table row
                $('.available-credits').html('Available Credits: <b>' + availableCredits + '</b>')
                ...
                if(availableCredits < totalRowsData)
            {
                var requiredCredits=totalRowsData - availableCredits;
    ...
    function submitData(properties, headersUpdated = false) {
        return new Promise((resolve, reject) => { // Return a promise
            $.ajax({
                ...
                data: JSON.stringify({ properties, headersUpdated, fileNumber }), // Send both properties and headersUpdated
